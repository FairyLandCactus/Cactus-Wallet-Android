...
import com.qianlihu.solanawallet.adapter.DappEthListAdapter;
 ...
 * version: 1.0.0
 */
public class DappEthFagment extends BaseFragment<DappPresenter> implements DappView {
    ...
    private int page = 1, pageSize = 10, total = 0, id = 4;

    public static Fragment newInstance(int id, int status) {
        DappEthFagment fragment = new DappEthFagment();
    ...
    public int getLayoutId() {
        return R.layout.fragment_dapp_eth;
    }
    ...
    private void showSolDappUI(List<DappTypeBean.DataBean.RowsBean> beanList) {
        ...
        listAdapter.setOnItemClickListener((adapter, view, position) -> DialogUtils.junmpDappDialog(getContext(),3,beanList.get(position)));
