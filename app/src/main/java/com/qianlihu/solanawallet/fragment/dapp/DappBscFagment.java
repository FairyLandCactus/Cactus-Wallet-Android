 ...
 * version: 1.0.0
 */
public class DappBscFagment extends BaseFragment<DappPresenter> implements DappView {
    ...
    private int page = 1, pageSize = 10, total = 0, id = 3;

    public static Fragment newInstance(int id, int status) {
        DappBscFagment fragment = new DappBscFagment();
    ...
    public int getLayoutId() {
        return R.layout.fragment_dapp_bsc;
    }
    ...
    private void showSolDappUI(List<DappTypeBean.DataBean.RowsBean> beanList) {
        ...
        listAdapter.setOnItemClickListener((adapter, view, position) -> DialogUtils.junmpDappDialog(getContext(),2,beanList.get(position)));
