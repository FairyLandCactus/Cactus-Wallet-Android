 ...
 * version: 1.0.0
 */
public class UpdateBnbTransferRecordInfo {
    ...
    public void transferRecord(String mPuk, String walletType, String mints, String name, int page, int pageSize) {
        ...
        List<BNBTransferRecordDB> dbs = LitePal.where("myPuk = ? and walletType = ? and contractAddress = ?", mPuk, walletType, mints.toLowerCase())
        ...
        BinanceWalletUtil.getBlockNumber().subscribe(new SingleObserver<BigInteger>() {
            @Override
            ...
            public void onSuccess(BigInteger bigInteger) {
                ...
                if (name.equals(Constant.BNB_CHAIN)) {
                    getBnbTransferRecord(endBlock, mPuk, walletType, mints, page, pageSize);
                } else {
    ...
    private void getBnbTransferRecord(int endblock, String mPuk, String mWalletType, String mint, int page, int pageSize) {
        ...
        linkedHashMap.put("action", "txlistinternal");
        ...
        linkedHashMap.put("apikey", Constant.BSCSCAN_APIKEY);

        HttpService.doGet(Constant.BSCSCAN_URL, linkedHashMap, new Callback() {
            @Override
    ...
    private void bnbSave(BnbTransferRecordBean bean, String mPuk, String mWalletType, String mint) {
        for (int i = 0; i < bean.getResult().size(); i++) {
            ...
            String values = bean.getResult().get(i).getValue();
            List<BNBTransferRecordDB> dbs = LitePal.where("myPuk = ? and hash = ? and value = ?", mPuk, hash,values).find(BNBTransferRecordDB.class);
            if (dbs.size() == 0) {
                String contract = bean.getResult().get(i).getContractAddress();
                if (TextUtils.isEmpty(contract)) {
                    contract = Constant.BNB_TOKEN_ADDRESS;
                }
                BNBTransferRecordDB db = new BNBTransferRecordDB();
                db.setMyPuk(mPuk);
        ...
        List<BNBTransferRecordDB> dbs = LitePal.where("myPuk = ? and walletType = ? and contractAddress = ?", mPuk, mWalletType, mint.toLowerCase())
                .order("timeStamp desc")
                .limit(10)
                .find(BNBTransferRecordDB.class);
