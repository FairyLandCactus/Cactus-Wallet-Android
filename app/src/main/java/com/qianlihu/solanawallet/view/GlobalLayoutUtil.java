    ...
    private void possiblyResizeChildOfContent(Activity activity) {
        ...
        if (usableHeightNow != usableHeightPrevious) {
            ...
            if (heightDifference > usableHeightSansKeyboard / 4) {
                EventBus.getDefault().post(new HideMianTabEvent(true));
                // 设置布局高度为：屏幕高度 - 高度差
                frameLayoutParams.height = usableHeightSansKeyboard - heightDifference + navHeight;
            } else if (heightDifference >= navHeight) {
                EventBus.getDefault().post(new HideMianTabEvent(false));
                // keyboard probably just became hidden
                // 虚拟导航栏显示
                frameLayoutParams.height = usableHeightSansKeyboard - navHeight;
            } else {// 其他情况直接设置为可视高度即可
                frameLayoutParams.height = usableHeightNow - heightDifference + navHeight;
                EventBus.getDefault().post(new HideMianTabEvent(false));
            }
        }
        // 刷新布局，会重新测量、绘制
